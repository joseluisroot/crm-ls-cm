                    ...
                    <?php foreach ($participants as $index => $person): ?>
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <p class="text-xs text-slate-400 mt-1"><?= esc($person['external_id']) ?></p>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex px-3 py-1 rounded-full bg-pink-50 text-pink-700 font-black">
                                    <?= esc($person['total_interactions']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-500"><?= esc($person['last_interaction_at'] ?: 'Sin fecha') ?></td>
                        </tr>
                    <?php endforeach; ?>

                    <?php if (empty($participants)): ?>
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">Sin datos de participación todavía.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <aside class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm h-fit">
        ...
        <div class="mt-5 space-y-4">
            <?php foreach ($reactionBreakdown as $reaction): ?>
                <div class="flex items-center justify-between gap-4 border-b border-slate-100 pb-4 last:border-0 last:pb-0">
                    <span class="font-bold text-slate-700"><?= esc($reaction['reaction_type']) ?></span>
                    <span class="px-3 py-1 rounded-full bg-slate-100 text-slate-700 font-black">
                        <?= esc($reaction['total']) ?>
                    </span>
                </div>
            <?php endforeach; ?>
            ...
            <?php if (empty($reactionBreakdown)): ?>
                <p class="text-slate-500">Sin reacciones registradas.</p>
            <?php endif; ?>
        </div>
    </aside>
</div>
