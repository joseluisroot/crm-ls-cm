...
final class OperationsDetailQueryService
{
    ...
    public function find(int $id): ?array
    {
        ...
        if (($item['origin_type'] ?? '') === 'FACEBOOK_COMMENT') {
            $item['source'] = $this->connection()->table('social_comments sc')
                ...
                ->where('sc.external_comment_id', $item['origin_id'])
                ->get()
                ->getRowArray();
        } else {
            $item['source'] = null;
    ...
    {
        return $this->connection()->table('system_events')
    ...
    {
        return $this->connection()->table('admin_users')
    ...
    {
        return $this->catalog('work_item_statuses');
    }

    public function priorities(): array
    {
        return $this->catalog('work_item_priorities');
    }
