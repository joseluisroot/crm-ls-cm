...
<div class="min-h-screen flex">
    <aside class="w-72 bg-slate-950 text-white p-6 flex flex-col shrink-0">
        ...
        <div class="mt-auto pt-8"><div class="border-t border-slate-800 pt-6"><p class="text-xs uppercase tracking-widest text-slate-500">Usuario activo</p><p class="font-semibold mt-1"><?= esc(session()->get('admin_user_name') ?? 'Usuario') ?></p><div class="inline-flex mt-2 mb-5 px-3 py-1 rounded-full border text-xs font-bold <?= $profileClass ?>"><?= esc($profile['label']) ?></div><form method="post" action="<?= site_url('admin/logout') ?>" data-confirm="¿Deseas cerrar sesión?" data-confirm-text="Tu sesión administrativa finalizará de forma segura." data-confirm-button="Sí, cerrar sesión" data-loading="Cerrando sesión..."><?= csrf_field() ?><button type="submit" class="block w-full text-center px-4 py-3 rounded-xl bg-slate-800 hover:bg-red-600 transition font-semibold">Cerrar sesión</button></form><div class="mt-6 text-center"><p class="text-xs text-slate-500">CIAC Platform</p><p class="text-xs text-slate-600 mt-1">Version 1.4 Operational Foundation</p></div></div></div>
    </aside>
